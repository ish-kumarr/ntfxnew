  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        // Defensive check: ensure context AND its attributes exist
        // The postprocessing EffectComposer internally reads getContextAttributes().alpha
        // which crashes if getContextAttributes() returns null
        const gl = this.renderer.getContext();
        if (!gl || !gl.getContextAttributes()) {
          console.warn("Hyperspeed: WebGL context or attributes unavailable, skipping post-processing.");
          ...
          try {
            ...
            console.warn("Hyperspeed: EffectComposer failed to initialize, falling back to basic rendering.", e);
            this.composer = null;
          }
        }
      ...
      initPasses() {
        if (!this.hasContext || !this.composer || !this.renderer) return;

        // The postprocessing library's addPass() internally reads
        // gl.getContextAttributes().alpha — if getContextAttributes() is null
        // (context lost, browser resource limit, etc.) it throws.
        // We MUST check this before calling addPass.
        const gl = this.renderer.getContext();
        if (!gl || !gl.getContextAttributes()) {
          console.warn("Hyperspeed: getContextAttributes() is null, skipping post-processing.");
        ...
        try {
          ...
          console.warn("Hyperspeed: Post-processing failed, falling back to basic rendering.", e);
          if (this.composer) {
            this.composer.dispose();
            this.composer = null;
          }
      ...
      init() {
        ...
        // Use mouseenter and mouseleave for hover interactions per user request
        this.container.addEventListener('mouseenter', this.onMouseDown);
      ...
      dispose() {
        ...
        if (this.container) {
          this.container.removeEventListener('mouseenter', this.onMouseDown);
